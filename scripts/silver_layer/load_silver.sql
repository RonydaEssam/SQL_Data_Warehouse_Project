...
SCRIPT PURPOSE:
	This script is a stored procedure used to:
	 - Truncate the tables of the silver layer.
	 - Load the data from the tables os the bronze layer into the tables of the silver layer using 'INSERT INTO ..'.
...
BEGIN
	...
	BEGIN TRY
		...
		PRINT '>> Truncating Table: silvesr.crm_cust_info';
		...
		-- Load data into table: silver.erp_cust_za12
		SET @start_time = GETDATE();
		...
		-- Load data into table: silver.erp_loc_a101
		SET @start_time = GETDATE();
		...
		-- Load data into table: silver.erp_px_cat_g1v2
		SET @start_time = GETDATE();
